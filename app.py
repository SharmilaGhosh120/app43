...
OPENAI_API_KEY = os.getenv('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')
...
def reset_user_password(email, new_password):
    try:
        if not is_strong_password(new_password):
            return False, "Password must be at least 8 characters long, include one uppercase letter and one digit."
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
...
def get_project_for_student(email, conn):
    try:
        ...
        return "Error retrieving project."
...
def save_query(email, name, question, response, conn, feedback_rating=None):
    try:
...
def export_query_logs_to_csv(student_email=None, start_date=None, end_date=None):
    try:
        conn = sqlite3.connect(DB_PATH)
        query = """
            SELECT q.email, q.name, q.project_title, q.question, q.response, q.email as email, q.feedback_rating
            FROM queries q
            WHERE q.email IN (SELECT email FROM users WHERE role != 'admin')
        """
        params = []
        if student_email:
            query += " AND q.email = ?"
...
def export_project_mappings_to_csv():
    try:
        ...
        """
        project_mappings = pd.read_sql_query(query, conn)
        conn.close()
        ...
        return csv_data, "Project mappings exported successfully!"
    except sqlite3.Error as e:
        logger.error(f"Database error in export_project_mappings_to_csv: {str(e)}")
        return None, "Database error while fetching project mappings."
...
def plot_feedback_chart():
    try:
        ...
        plt.hist(feedback_df['feedback_rating'], bins=5, edgecolor='black')
...
if not st.session_state.authenticated:
    ...
    def show_admin_dashboard(name):
                ...
                if not all(col in mapping_df.columns for col in required_columns):
                    ...
                    if st.button("Save Mapping"):
                        ...
                        try:
                            ...
                            st.error("One or more mappings already exist.")
                        except sqlite3.Error as e:
        ...
        if st.button("Export Project Mappings to CSV"):
            with st.spinner("Exporting CSV..."):
                csv_data, message = export_project_mappings_to_csv()
                if csv_data:
                    st.download_button(
                        label="Download Project Mappings CSV",
                        data=csv_data,
                        file_name=f"project_mappings_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                        mime="text/csv"
                    )
                    st.success(message)
                else:
        ...
        conn = sqlite3.connect(DB_PATH)
        query_logs = pd.read_sql_query("SELECT q.email, q.name, q.project_title, q.question, q.response, q.timestamp, q.feedback_rating FROM queries q WHERE q.email IN (SELECT email FROM users WHERE role != 'admin')", conn)
        conn.close()
        if query_logs.empty:
            query_logs = pd.DataFrame({
    ...
    def kyra_response(email, query):
        try:
            ...
            st.error("Unable to connect to Ky’ra’s AI. Please try again later.")
            return None
